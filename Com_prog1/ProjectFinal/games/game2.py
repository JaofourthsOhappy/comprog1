...
class Board :
    def __init__(self,money):
        """ receive money as input and initilize Board 
        """
    ...
    def draw(self,screen,choose):
        """ display every image to user interface
        """
        ...
        life = font.render(f"Your life is {self.__money}" , 1 , (255,255,255))
    ...
    def print_grid(self):
        """ print self.__grid to check its status
        """
        print(self.__grid[2])

    def get_mouse(self, x, y , choose,screen):
        """ get a position that mouse clicked 
            set __grid to 'X' if position is in betting panel 
    ...
    def check_grid(self,x,y,choose):
        """ check the win condition and update values from win and lose
        """
    ...
    def end_game(self):
        """ return True if your money is less than 0
        """
        if self.__money <=0 :
            return True
        else: 
    ...
    def winning(self):
        """ update money when win
        """
        self.__money +=1
    def losing(self):
        """ update money when lose
        """
        self.__money -= 1

    def right_choice(self):
        """ create and return list of right choice
        """
        import random
        choose_list = []
        for i in range(3):
    ...
    def play_again(self,x,y):
        """ return 1 if user click play again button
            return 0 if user want to quit
        """
        if x in [1,2] and y ==  2:
            return 1
    ...
    def OK (self,x,y):
        """ return True if user click play button return False otherwise
        """
        ok_botton_pos_x = 5
        ok_botton_pos_y = 0
    ...
    def count_place(self):
        """ count self.__grid and return its value
        """
        num_x = 0
    ...
    def main(self): 
        ...
        while run :
            ...
            if self.board.end_game():
                font = pygame.font.SysFont("comicsans" , 64 ,True)
                game_over = font.render("You don't have money to play" , 1 , (255,255,255))
