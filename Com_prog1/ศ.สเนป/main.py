#* : option that allows player to draw more cards if he does not want to stand
import sys
from BJ import initialize_cards, shuffle_cards , display_cards ,draw_cards ,calculate_hand_value,check_for_Blackjack,must_draw_more,option 
...
while (play_again):
    ...
    # An option if you want more card
    more_card = option("More card? ")
    while (more_card):
        player_hand += draw_cards(deck,1)
        print("Your hand: ", end = '')
        display_cards(player_hand)
    ...
    play_again = option("Play a new round : ")
